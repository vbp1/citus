					...
					RecursivePlanningContext *recursivePlanningContext)
{
	...
	if (IsA(recurringNode, RangeTblRef))
	{
        ...
        RangeTblEntry *joinRte = rt_fetch(joinExpr->rtindex,
                                               currentQuery->rtable);

		distributedRte->subquery =
			WrapSubqueryInInnerJoin(recurringRte, joinExpr, joinRte, distributedRte);
	}
						...
						RangeTblEntry *distributedRte)
{
    ...
    recurringRef->rtindex = 1;

    RangeTblRef *subqueryRef = makeNode(RangeTblRef);
    subqueryRef->rtindex = 2;

    JoinExpr *innerJoin = copyObject(joinExpr);
    innerJoin->jointype = JOIN_INNER;
    innerJoin->larg = (Node *) recurringRef;
    innerJoin->rarg = (Node *) subqueryRef;
    innerJoin->rtindex = 3;

    RangeTblEntry *innerJoinRte = copyObject(joinRte);
    innerJoinRte->jointype = JOIN_INNER;
    innerJoinRte->rtekind = RTE_JOIN;

    query->rtable = list_make3(newRecurringRte, newDistributedRte, innerJoinRte);

    query->jointree = makeFromExpr(list_make1(innerJoin), NULL);

	query->targetList =
		copyObject(distributedRte->subquery->targetList);

    /* indirectly created by CreateFilteredTargetListForRelation, need to fix due to SINGLE_RTE_INDEX  */
    TargetEntry *te = NULL;
    foreach_ptr(te, query->targetList)
    {
        ((Var *) te->expr)->varno = 2;
    }
